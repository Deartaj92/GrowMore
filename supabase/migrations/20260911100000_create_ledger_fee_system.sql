...
ALTER TABLE public.cashier_shifts ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ DEFAULT now();
...
-- ------------------------------------------------------------------------------
-- RPC 1B: ATOMIC MULTI-CHALLAN STUDENT PAYMENT COLLECTION (fn_collect_student_multi_payment)
-- ------------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.fn_collect_student_multi_payment(
    p_school_id BIGINT,
    p_student_id BIGINT,
    p_received_amount NUMERIC(12,2),
    p_payment_method VARCHAR(30),
    ...
    p_payer_name VARCHAR(255) DEFAULT NULL,
    p_shift_id BIGINT DEFAULT NULL,
    p_selected_demand_ids BIGINT[] DEFAULT NULL
)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
    ...
    v_demand_allocated NUMERIC(12,2);
    v_item_allocated NUMERIC(12,2);
    v_receipt_id BIGINT;
    v_receipt_no VARCHAR(60);
    v_student_balance NUMERIC(12,2);
    v_new_running_balance NUMERIC(12,2);
    v_receipt_snapshot JSONB;
    v_allocations_list JSONB := '[]'::JSONB;
    v_demands_list JSONB := '[]'::JSONB;
    v_total_student_due NUMERIC(12,2) := 0.00;
    v_demands_paid_count INT := 0;
    v_first_demand_id BIGINT;
BEGIN
    -- 1. Check Idempotency Key
    IF p_idempotency_key IS NOT NULL THEN
        SELECT id, receipt_no, status, receipt_snapshot INTO v_receipt_id, v_receipt_no, v_student_balance, v_receipt_snapshot
        FROM public.fee_receipts
        WHERE school_id = p_school_id AND idempotency_key = p_idempotency_key;
    ...
    -- 3. Calculate Total Due & Get Unpaid Demands Chronologically
    SELECT COALESCE(SUM(total_balance_amount), 0.00) INTO v_total_student_due
    FROM public.fee_demands
    WHERE school_id = p_school_id
      AND student_id = p_student_id
      AND status != 'void'
      AND total_balance_amount > 0
      AND (p_selected_demand_ids IS NULL OR id = ANY(p_selected_demand_ids));

    IF v_total_student_due <= 0 THEN
        RAISE EXCEPTION 'Student has no outstanding fee demands to pay.';
    END IF;
    ...
    IF p_received_amount > v_total_student_due THEN
        RAISE EXCEPTION 'Payment amount (Rs. %) exceeds total outstanding balance (Rs. %).', p_received_amount, v_total_student_due;
    END IF;
    ...
    -- Get first demand ID for receipt foreign key reference
    SELECT id INTO v_first_demand_id
    FROM public.fee_demands
    WHERE school_id = p_school_id AND student_id = p_student_id AND status != 'void' AND total_balance_amount > 0
      AND (p_selected_demand_ids IS NULL OR id = ANY(p_selected_demand_ids))
    ORDER BY due_date ASC, id ASC LIMIT 1;

    -- 5. Insert Receipt Master Record
    INSERT INTO public.fee_receipts (
        school_id, receipt_no, shift_id, student_id, demand_id,
        payment_date, payment_method, received_amount, reference_no, bank_name,
        payer_name, collector_user_id, idempotency_key, status
    ) VALUES (
        p_school_id, v_receipt_no, p_shift_id, p_student_id, v_first_demand_id,
        now(), p_payment_method, p_received_amount, p_reference_no, p_bank_name,
        COALESCE(p_payer_name, v_student.name), p_collector_user_id, p_idempotency_key, 'posted'
    ) RETURNING id INTO v_receipt_id;

    v_remaining_amount := p_received_amount;

    -- 6. Loop over Unpaid Demands in Chronological Order (Oldest Due Date First)
    FOR v_demand IN
    ...
    LOOP
        ...
        v_demand_allocated := 0.00;

        -- Loop through line items of this demand ordered by priority
        FOR v_item IN
            SELECT * FROM public.fee_demand_items
            WHERE demand_id = v_demand.id AND school_id = p_school_id AND balance_amount > 0
            ORDER BY priority_order ASC, id ASC
            FOR UPDATE
        LOOP
            IF v_remaining_amount <= 0 THEN
            ...
            IF v_remaining_amount >= v_item.balance_amount THEN
                v_item_allocated := v_item.balance_amount;
            ELSE
                v_item_allocated := v_remaining_amount;
            END IF;

            -- Record line item allocation
            INSERT INTO public.fee_allocations (
                school_id, receipt_id, demand_item_id, allocated_amount, allocation_timestamp
            ) VALUES (
                p_school_id, v_receipt_id, v_item.id, v_item_allocated, now()
            );

            -- Update demand item paid, balance, status
            UPDATE public.fee_demand_items
            SET paid_amount = paid_amount + v_item_allocated,
                balance_amount = balance_amount - v_item_allocated,
                status = CASE WHEN (balance_amount - v_item_allocated) = 0 THEN 'paid' ELSE 'partially_paid' END,
                updated_at = now()
            WHERE id = v_item.id;

            v_demand_allocated := v_demand_allocated + v_item_allocated;
            v_remaining_amount := v_remaining_amount - v_item_allocated;

            v_allocations_list := v_allocations_list || jsonb_build_object(
                'demand_id', v_demand.id,
                'challan_no', v_demand.challan_no,
                'item_id', v_item.id,
                'head_title', v_item.head_title_snapshot,
                'allocated', v_item_allocated,
                'remaining_balance', v_item.balance_amount - v_item_allocated
            );
        END LOOP;

        -- Update Demand master totals & status
        UPDATE public.fee_demands
        SET total_paid_amount = total_paid_amount + v_demand_allocated,
            total_balance_amount = total_balance_amount - v_demand_allocated,
            status = CASE WHEN (total_balance_amount - v_demand_allocated) <= 0 THEN 'paid' ELSE 'partially_paid' END,
            updated_at = now()
        WHERE id = v_demand.id;

        v_demands_list := v_demands_list || jsonb_build_object(
            'demand_id', v_demand.id,
            'challan_no', v_demand.challan_no,
            'billing_month', v_demand.billing_month,
            'billing_year', v_demand.billing_year,
            'allocated', v_demand_allocated,
            'remaining_demand_balance', v_demand.total_balance_amount - v_demand_allocated
        );

        v_demands_paid_count := v_demands_paid_count + 1;
    END LOOP;

    -- 7. Update Shift Drawer Collections if active shift specified
    IF p_shift_id IS NOT NULL THEN
        IF p_payment_method = 'cash' THEN
            UPDATE public.cashier_shifts
            SET total_cash_collected = total_cash_collected + p_received_amount
    ...
    -- 8. Post Single Consolidated Double-Entry Subledger Entry (PAYMENT)
    SELECT COALESCE(running_balance, 0.00) INTO v_student_balance
    FROM public.fee_ledger_entries
    ...
    INSERT INTO public.fee_ledger_entries (
        school_id, student_id, receipt_id, entry_type, entry_date,
        debit_amount, credit_amount, running_balance, description, performed_by
    ) VALUES (
        p_school_id, p_student_id, v_receipt_id, 'PAYMENT', CURRENT_TIMESTAMP,
        0.00, p_received_amount, v_new_running_balance,
        'Fee Payment collected (Receipt #' || v_receipt_no || ') allocated across ' || v_demands_paid_count || ' challan(s)',
        p_collector_user_id
    );

    -- Build Complete Receipt Snapshot for Instant Thermal / A4 Print
    v_receipt_snapshot := jsonb_build_object(
        'receipt_id', v_receipt_id,
        'receipt_no', v_receipt_no,
        ...
        'received_amount', p_received_amount,
        'reference_no', p_reference_no,
        'payer_name', COALESCE(p_payer_name, v_student.name),
        'collector_id', p_collector_user_id,
        'demands_applied', v_demands_list,
        'item_allocations', v_allocations_list,
        'previous_balance', v_total_student_due,
        'remaining_balance', v_total_student_due - p_received_amount
    );
    ...
    RETURN jsonb_build_object(
        ...
        'receipt_no', v_receipt_no,
        'received_amount', p_received_amount,
        'demands_count', v_demands_paid_count,
        'total_due_before', v_total_student_due,
        'remaining_due_after', v_total_student_due - p_received_amount,
        'snapshot', v_receipt_snapshot
    );
END;
$$;
...
DECLARE
    ...
    v_total_active_students INTEGER := 0;
    v_student_snapshot JSONB;
    v_existing_demand_id BIGINT;
    v_default_head_id BIGINT;
    v_policy_count INTEGER;
BEGIN
    -- 1. Sync & update active students from `students` into `student_enrollments` if missing or class changed
    ...
    INSERT INTO public.student_enrollments (
        school_id, student_id, session_id, class_id, section_id, roll_number, enrollment_type, status
    )
    SELECT 
        s.school_id, s.id, p_session_id, s.class_id, s.section_id, COALESCE(s.roll_number, ''), 'new_admission', 'active'
    FROM public.students s
    WHERE s.school_id = p_school_id
      ...
      AND (s.status IS NULL OR s.status ILIKE 'active' OR s.status = '1' OR s.status ILIKE 'enrolled')
      AND NOT EXISTS (
          SELECT 1 FROM public.student_enrollments se 
          WHERE se.school_id = p_school_id AND se.student_id = s.id AND se.session_id = p_session_id
      )
    ON CONFLICT DO NOTHING;

    -- Also activate any existing student_enrollments if status was null
    UPDATE public.student_enrollments
    SET status = 'active'
    WHERE school_id = p_school_id AND session_id = p_session_id AND status IS NULL;

    -- 2. Ensure at least one Fee Head exists for the school
    SELECT id INTO v_default_head_id
    ...
    IF NOT FOUND THEN
        ...
        VALUES (p_school_id, 'TUIT', 'Tuition Fee', 'academic', false, true)
        RETURNING id INTO v_default_head_id;
    END IF;

    -- 3. Ensure at least one Fee Policy Version exists for this session
    SELECT COUNT(*) INTO v_policy_count
    FROM public.fee_policy_versions
    WHERE school_id = p_school_id AND session_id = p_session_id AND status = 'active';

    IF v_policy_count = 0 THEN
        INSERT INTO public.fee_policy_versions (
            school_id, fee_head_id, version_number, session_id, frequency,
            amount_rule, base_amount, due_day_of_month, grace_period_days,
            late_fine_type, late_fine_amount, max_late_fine, status
        ) VALUES (
            p_school_id, v_default_head_id, 1, p_session_id, 'monthly',
            'fixed', 1000.00, 10, 5,
            'fixed', 100.00, 500.00, 'active'
        );
    END IF;

    -- 4. Count total active students matching target
    ...
    WHERE e.school_id = p_school_id
      ...
      AND (p_section_id IS NULL OR e.section_id = p_section_id OR s.section_id = p_section_id)
      AND (p_student_id IS NULL OR e.student_id = p_student_id OR s.id = p_student_id)
      AND (e.status ILIKE 'active' OR e.status = '1');

    -- 5. Iterate over active enrollments matching criteria
    FOR v_enrollment IN
        SELECT e.*, s.name as student_name, s.father_name, s.phone as student_phone,
               COALESCE(c.name, c2.name) as class_name, COALESCE(sec.name, sec2.name) as section_name
        FROM public.student_enrollments e
        JOIN public.students s ON s.id = e.student_id AND s.school_id = e.school_id
        LEFT JOIN public.classes c ON c.id = e.class_id AND c.school_id = e.school_id
        LEFT JOIN public.classes c2 ON c2.id = s.class_id AND c2.school_id = s.school_id
        LEFT JOIN public.sections sec ON sec.id = e.section_id AND sec.school_id = e.school_id
        LEFT JOIN public.sections sec2 ON sec2.id = s.section_id AND sec2.school_id = s.school_id
        WHERE e.school_id = p_school_id
          AND e.session_id = p_session_id
          AND (p_class_id IS NULL OR e.class_id = p_class_id OR s.class_id = p_class_id)
          AND (p_section_id IS NULL OR e.section_id = p_section_id OR s.section_id = p_section_id)
    ...
    LOOP
        ...
        v_student_snapshot := jsonb_build_object(
            ...
            'class_id', COALESCE(v_enrollment.class_id, (SELECT class_id FROM public.students WHERE id = v_enrollment.student_id)),
            'class_name', COALESCE(v_enrollment.class_name, 'Class ' || v_enrollment.class_id),
        ...
        FOR v_policy IN
            ...
            WHERE p.school_id = p_school_id
              ...
              AND p.status = 'active'
              AND (p.class_id IS NULL OR p.class_id = v_enrollment.class_id OR p.class_id = (SELECT class_id FROM public.students WHERE id = v_enrollment.student_id))
              AND (p.section_id IS NULL OR p.section_id = v_enrollment.section_id OR p.section_id = (SELECT section_id FROM public.students WHERE id = v_enrollment.student_id))
        LOOP
            ...
            INSERT INTO public.fee_demand_items (
                school_id, demand_id, fee_head_id, policy_version_id,
                head_title_snapshot, gross_amount, concession_amount, late_fine_amount,
                waiver_amount, net_amount, paid_amount, balance_amount, status
            ) VALUES (
                p_school_id, v_demand_id, v_policy.fee_head_id, v_policy.id,
                v_policy.head_title, v_gross, v_concession_amt, 0.00,
                0.00, v_net, 0.00, v_net, 'unpaid'
            );
        ...
        -- Post Subledger Journal Debit Entry (DEMAND)
        INSERT INTO public.fee_ledger_entries (
            school_id, student_id, demand_id, entry_type, entry_date,
            debit_amount, credit_amount, running_balance, description, performed_by
        ) VALUES (
            p_school_id, v_enrollment.student_id, v_demand_id, 'DEMAND', CURRENT_TIMESTAMP,
            v_total_net, 0.00,
            COALESCE((
                SELECT running_balance FROM public.fee_ledger_entries 
                WHERE school_id = p_school_id AND student_id = v_enrollment.student_id 
                ORDER BY id DESC LIMIT 1
            ), 0.00) + v_total_net,
            'Monthly Demand Billed (' || p_billing_month || '/' || p_billing_year || ') Challan #' || v_challan_no,
            p_created_by
        );
    ...
    RETURN jsonb_build_object(
        ...
        'total_target_students', v_total_active_students,
        'message', 'Generated ' || v_generated_count || ' demands (' || v_already_exists_count || ' already existed).'
    );
END;
$$;

-- ------------------------------------------------------------------------------
-- RPC: GENERATE FAMILY CONSOLIDATED DEMANDS (fn_generate_family_monthly_demands)
-- ------------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.fn_generate_family_monthly_demands(
    p_school_id BIGINT,
    p_family_id BIGINT,
    p_session_id INTEGER,
    p_billing_month INTEGER,
    ...
    p_due_date DATE DEFAULT CURRENT_DATE + INTERVAL '10 days',
    p_validity_date DATE DEFAULT CURRENT_DATE + INTERVAL '25 days',
...
BEGIN
    -- Find all active students belonging to target family
    FOR v_sibling IN
    ...
    LOOP
        ...
        v_res := public.fn_generate_monthly_demands(
            p_school_id => p_school_id,
            p_session_id => p_session_id,
            p_class_id => NULL,
            p_section_id => NULL,
            p_billing_month => p_billing_month,
            p_billing_year => p_billing_year,
            p_issue_date => p_issue_date,
            p_due_date => p_due_date,
            p_validity_date => p_validity_date,
            p_created_by => p_created_by,
            p_student_id => v_sibling.student_id
        );
        v_total_generated := v_total_generated + (v_res->>'generated_count')::INTEGER;
        v_already_exists := v_already_exists + (v_res->>'already_exists_count')::INTEGER;
    END LOOP;

    IF v_student_count = 0 THEN
        RAISE EXCEPTION 'No active students found for Family ID %', p_family_id;
    END IF;

    RETURN jsonb_build_object(
        'success', true,
        'family_id', p_family_id,
        'family_siblings_count', v_student_count,
        'generated_count', v_total_generated,
        'already_exists_count', v_already_exists,
        'message', 'Generated family fee demands for ' || v_student_count || ' siblings. (' || v_total_generated || ' new demands).'
    );
END;
$$;
...
-- ------------------------------------------------------------------------------
-- RPC: DELETE FEE DEMAND (UNCOLLECTED ONLY)
-- ------------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.fn_delete_fee_demand(
    p_school_id BIGINT,
    p_demand_id BIGINT,
...
BEGIN
    ...
    IF NOT FOUND THEN
        RAISE EXCEPTION 'Fee demand ID % not found.', p_demand_id;
    END IF;

    -- Enforce rule: Delete allowed ONLY if payment has NOT been made
    IF COALESCE(v_demand.total_paid_amount, 0.00) > 0 OR v_demand.status IN ('paid', 'partially_paid') THEN
        RAISE EXCEPTION 'Cannot delete Challan #% because payments totaling Rs. % have already been collected. Reverse/void the payment first.', v_demand.challan_no, v_demand.total_paid_amount;
    END IF;

    -- Check if any posted receipts reference this demand
    SELECT COUNT(*) INTO v_receipt_count
    FROM public.fee_receipts
    WHERE school_id = p_school_id AND demand_id = p_demand_id AND status = 'posted';

    IF v_receipt_count > 0 THEN
        RAISE EXCEPTION 'Cannot delete Challan #% because active payment receipts exist.', v_demand.challan_no;
    END IF;

    -- Delete associated ledger entry for demand issuance
    DELETE FROM public.fee_ledger_entries
    WHERE school_id = p_school_id AND demand_id = p_demand_id;

    -- Delete demand line items
    DELETE FROM public.fee_demand_items
    WHERE school_id = p_school_id AND demand_id = p_demand_id;

    -- Delete demand master record
    DELETE FROM public.fee_demands
    WHERE id = p_demand_id AND school_id = p_school_id;
    ...
    RETURN jsonb_build_object(
        ...
        'challan_no', v_demand.challan_no,
        'message', 'Challan #' || v_demand.challan_no || ' deleted successfully.'
    );
END;
$$;
