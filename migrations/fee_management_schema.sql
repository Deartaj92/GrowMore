-- This script defines the complete schema for fee management.
-- It combines the initial schema creation with all subsequent alterations.

-- Drop existing tables, triggers, and functions if they exist to ensure a clean slate
...
COMMENT ON COLUMN public.student_fee_concessions.effective_from IS 'Date from which the concession becomes effective';
COMMENT ON COLUMN public.student_fee_concessions.expires_on IS 'Date on which the concession expires and is no longer valid';
...
CREATE INDEX IF NOT EXISTS idx_student_fee_concessions_student ON public.student_fee_concessions(school_id, student_id);
...
-- Create triggers for updated_at columns (assumes update_updated_at_column() function exists)
