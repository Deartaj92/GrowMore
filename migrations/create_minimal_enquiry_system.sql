-- Minimal Enquiry System - Creates just the essential tables
-- This will stop the 400 errors and allow the enquiry system to work
...
INSERT INTO public.enquiry_statuses (name, description, color, sort_order) 
SELECT 'Approved', 'Inquiry approved/accepted', '#22c55e', 4
WHERE NOT EXISTS (SELECT 1 FROM public.enquiry_statuses WHERE name = 'Approved');

INSERT INTO public.enquiry_statuses (name, description, color, sort_order) 
SELECT 'Rejected', 'Inquiry rejected/declined', '#6b7280', 5
WHERE NOT EXISTS (SELECT 1 FROM public.enquiry_statuses WHERE name = 'Rejected');

INSERT INTO public.enquiry_statuses (name, description, color, sort_order) 
SELECT 'Resolved', 'Inquiry successfully resolved', '#059669', 6
