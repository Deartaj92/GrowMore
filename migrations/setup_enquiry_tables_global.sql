-- Global Enquiry System Setup
-- This script creates enquiry types that are available for all schools
...
-- Create enquiry_types table (global types, no school_id)
...
-- Create enquiry_statuses table (global statuses, no school_id)
...
-- Insert global enquiry types (available for all schools)
...
-- Insert global enquiry statuses (available for all schools)
...
-- Create RLS policies for global tables (enquiry_types and enquiry_statuses)
CREATE POLICY "All users can view enquiry types" ON public.enquiry_types
    FOR SELECT USING (is_active = true);

CREATE POLICY "All users can view enquiry statuses" ON public.enquiry_statuses
    FOR SELECT USING (is_active = true);
