-- Activity Tracking System for Teachers
...
-- Activity logs: Teachers can only see their own activities, admins can see all
CREATE POLICY "Teachers can view their own activities" ON activity_logs
    FOR SELECT USING (
        teacher_id = (SELECT id FROM staff WHERE user_id = auth.uid()) OR
        (SELECT role FROM staff WHERE user_id = auth.uid()) IN ('Super Admin', 'Principal', 'Admin')
    );

CREATE POLICY "Admins can insert activities" ON activity_logs
...
CREATE POLICY "Users can view their own notifications" ON notifications
    FOR SELECT USING (recipient_id = (SELECT id FROM staff WHERE user_id = auth.uid()));
...
CREATE POLICY "Users can manage their own preferences" ON notification_preferences
    FOR ALL USING (user_id = (SELECT id FROM staff WHERE user_id = auth.uid()));
...
-- 14. Create view for activity summary
