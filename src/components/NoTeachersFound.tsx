...
import DualTypingTextTeachers from './DualTypingTextTeachers';
...
const AddTeacherButton = muiStyled(Button)(({ theme }) => ({
...
export default function NoTeachersFound({ loading = false }: { loading?: boolean }) {
  ...
  return (
    <CenteredFlex>
      <IconCircle>
        <SchoolIcon style={{ fontSize: 54, color: theme === 'dark' ? darkTheme.ACCENT : lightTheme.ACCENT }} />
      </IconCircle>
      <DualTypingTextTeachers />
      <Typography
        variant="h6"
        fontWeight={600}
        sx={{
      ...
      >
        There are currently no teachers in the system. Start by adding your first teacher to unlock all features like class assignments, attendance, and more.
      </Typography>
      <AddTeacherButton
        size="large"
