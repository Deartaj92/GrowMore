...
const fineItems = [
  {
    title: 'Assign Fine',
    description: 'Set and assign fines for student violations, late arrivals, and disciplinary actions with flexible rules',
    icon: <MoneyIcon />,
    path: '/fines/assign',
    color: '#3b82f6' // Blue
  },
  {
    title: 'Collect Fine',
    description: 'Process fine payments, track collection status, and manage payment records with detailed reporting',
  ...
  },
  {
    ...
    description: 'View outstanding fines, track pending payments, and manage student debt with comprehensive analytics',
    icon: <WalletIcon />,
  ...
  },
  {
    ...
    description: 'Analyze fine trends, generate reports, and track collection performance with detailed insights',
    icon: <PieChartIcon />,
...
const FineDashboard: React.FC = () => {
  ...
  const getKeyForTitle = (title: string): string | null => {
    switch (title) {
      case 'Assign Fine': return 'fine_dash_assign';
  ...
  const visibleItems = useMemo(() => {
    if (user?.role !== 'Guest') return fineItems;
    if (!renderSettings) return fineItems;
    return fineItems.filter(item => {
  ...
  return (
    <PageContainer>
      <Header>
        ...
        <Subtitle>
          Comprehensive fine assignment, collection, and tracking system
        </Subtitle>
      </Header>
