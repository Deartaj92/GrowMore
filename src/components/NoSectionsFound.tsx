...
import DualTypingTextSections from './DualTypingTextSections';
...
const AddSectionButton = muiStyled(Button)(({ theme }) => ({
...
export default function NoSectionsFound({ loading = false }: { loading?: boolean }) {
  ...
  return (
    <CenteredFlex>
      <IconCircle>
        <ViewListIcon style={{ fontSize: 54, color: theme === 'dark' ? darkTheme.ACCENT : lightTheme.ACCENT }} />
      </IconCircle>
      <DualTypingTextSections />
      <Typography
        variant="h6"
        fontWeight={600}
        sx={{
      ...
      >
        There are currently no sections in the system. Start by adding your first section to organize students within classes and improve classroom management.
      </Typography>
      <AddSectionButton
        size="large"
